...
class CustomEnvironment(gym.Env):
    def __init__(self, model):
        ...
        self.state = np.array([32.0, 5.0]) + np.random.uniform(low=-0.5, high=0.5, size=(2,))

        # Init the current_step
        self.current_step = 0

        self.desire = np.array([32.715758, 5.027451724])
    ...
    def reset(self):
        ...
        if self.input_state is not None:
            self.state = np.array(self.input_state) + np.random.uniform(low=-1, high=1, size=(2,))  # 增加随机性
        else:
            self.state = np.array([32.0, 5.0]) + np.random.uniform(low=-1.0, high=1.0, size=(2,))
        return self.state
    ...
    def step(self, action):
        ...
        # eff_output_unscaled = scaler_eff.inverse_transform(eff_output.numpy().flatten())
...
def main():
    ...
    plt.axhline(5.027451724, color='r', linestyle='--', label='Eff_TP_desire')  # 添加目标值为0的水平线
    plt.xlabel('step')
    plt.ylabel('Eff')
    plt.legend()
    plt.show()
